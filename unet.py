...
class Unet(nn.Module):
    def __init__(self,  input_channels=3,num_classes=9, **kwargs):
        ...
        self.seg_head = SegmentationHead(nb_filter[0], num_classes)


    def forward(self, hyper, input):
        x0_0 = self.conv0_0(hyper, input)
        x1_0 = self.conv1_0(hyper, self.pool(x0_0))
        x2_0 = self.conv2_0(hyper, self.pool(x1_0))
        x3_0 = self.conv3_0(hyper, self.pool(x2_0))
        x4_0 = self.conv4_0(hyper, self.pool(x3_0))
